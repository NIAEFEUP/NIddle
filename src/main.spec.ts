...
describe('Main (bootstrap)', () => {
    ...
    it('should bootstrap the application successfully', async () => {
        ...
        // CRITICAL CHANGE:
        // 1. Check setup was called
        expect(mockSwaggerModule.setup).toHaveBeenCalledWith('docs', mockApp, expect.any(Function));

        // 2. Grab the function passed as the 3rd argument
        const documentFactory = mockSwaggerModule.setup.mock.calls[0][2];

        // 3. Execute it to trigger the code inside the arrow function
        documentFactory();

        // 4. Now verify createDocument ran
    ...
    it('should catch errors and exit process(1)', async () => {
        ...
        expect(consoleSpy).toHaveBeenCalledWith('Fatal error during bootstrap:', error);
