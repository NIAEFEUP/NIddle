...
export default setSeederFactory(Service, () => {
    ...
    const days: EnumDays[] = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'];
    const numberOfIntervals = faker.number.int({ min: 1, max: 5 });
    
    for (let i = 0; i < numberOfIntervals; i++) {
        const timeInterval = new TimeInterval();
        timeInterval.startTime = faker.date.soon({ refDate: '2024-01-01T08:00:00Z' });
        timeInterval.endTime = new Date(timeInterval.startTime.getTime() + faker.number.int({ min: 1, max: 4 }) * 60 * 60 * 1000);
