...
class ActiveAssociationGuard implements CanActivate {
    canActivate(context: ExecutionContext): boolean {
        ...
        if (isNaN(activeAssociationId)) {
            throw new BadRequestException('Active Association header must be a valid integer.');
        }
        ...
        const hasAssociation = user.associations?.some((association: { id: number; }) => association.id === activeAssociationId);

        if (!hasAssociation) {
            throw new ForbiddenException(`User does not have access to association with ID ${activeAssociationId}.`);
        }
